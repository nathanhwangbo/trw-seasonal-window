...
model {
  ...
  // ar
  /////
  vector[N_months] clim_monthly_anom;
  for(t in 1:N_months){
    clim_monthly_anom[t] = clim_monthly[t] - mu;
  }

  vector[N_months-1] delta;
  delta = clim_monthly_anom[2:N_months] - alpha_ar * clim_monthly_anom[1:(N_months-1)];
  for(t in 1:size(delta)){
    delta[t] ~ normal(0, sigma_ar);
  }
...
generated quantities {
  ...
  { // creating a block -- pos won't be a parameter
    ...
    for(t in 1:N_years){
      ...
      for(m in 1:12){
        clim_monthly_raw[pos + m - 1] = clim_monthly[pos + m - 1]*month_sds_mod[m] + month_means_mod[m];
        // clim_monthly_raw = exp(clim_monthly_log);

      }

      // what we want: un-transform
      clim_jja[t] = dot_product(w_jja, segment(clim_monthly_raw, pos, 12));
