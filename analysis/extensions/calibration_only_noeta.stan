...
transformed parameters {
  ...
  // vector[N_months_obs] clim_monthly;
  vector[N_years_obs] clim_w_yearly; // weighted monthly clim
  ...
  { // creating a block -- pos won't be a parameter
    int pos;
    pos = 1;
    for(t in 1:N_years_obs){
      // what we're modelling
      clim_w_yearly[t] = dot_product(w, segment(clim_obs, pos, 12));
      pos = pos + 12;
    }
  }


  // vector[N_years_obs] eta;
  //
  // array[N_years_obs] real<lower=0> sigma_eta_vec;
  // sigma_eta_vec[1] = sigma_eta;
  // eta[1] = eta_eps[1] * sigma_eta;
  // for(t in 2:N_years_obs){
...
model {
  ...
  pos = 1;
  for(i in 1:N_trees){
    segment(trw, pos, s[i]) ~ normal(beta0[i] +  beta1*segment(clim_w_yearly, start_index[i], s[i]) + beta_age[i] * segment(age, pos, s[i]), sigma[i]);
    pos = pos + s[i];
  }

  // clim_monthly[2:N_months_obs] ~ normal(mu + alpha_ar*(clim_monthly[1:N_months-1] - mu), sigma_ar);
  clim_obs[2:N_months_obs] ~ normal(mu + alpha_ar*(clim_obs[1:N_months_obs-1] - mu), sigma_ar);
