...
// here, the input is a CHRONOLOGY, rather than raw measurements
//
...
data {
  ...
  int<lower=0> start_year; // start year of the chronology
  vector[N_years] trw; // long vector with all trees concatenated
...
model {
  ...
  trw ~ normal(beta0 + beta1*clim_w_yearly, sigma);
  // int pos;
  // pos = 1;
  // for(i in 1:N_trees){
